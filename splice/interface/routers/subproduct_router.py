...
from splice.interface.schemas.subproduct_schema import (
    SubproductCreateSchema,
    SubproductResponseSchema,
...
async def get(
    ...
    if not subproduct:
        raise HTTPException(status_code=404, detail='Subproduct não encontrado')

    return subproduct


@router.post('', response_model=SubproductResponseSchema)
async def create(
    data: SubproductCreateSchema = Body(),
    db_session: Session = Depends(get_session)
):
    repo = SubproductRepository(db_session)
    service = SubproductService(repo)
    subproduct = await service.create(
...
@router.put('', response_model=SubproductResponseSchema)
async def update(
    data: SubproductUpdateSchema = Body(), db_session: Session = Depends(get_session)
):
    repo = SubproductRepository(db_session)
    service = SubproductService(repo)
...
@router.delete('')
async def delete(subproduct_id: str, db_session=Depends(get_session)):
    repo = SubproductRepository(db_session)
    service = SubproductService(repo)
