...
class NotificationRepository:
    ...
    async def save(self, data: NotificationCreateSchema) -> NotificationResponseSchema:
        try:
            result = await self.collection.insert_one(data.model_dump())
            if result:
                return NotificationResponseSchema(id=str(result.inserted_id), is_read=data.is_read, message_id=data.message_id, user_id=data.user_id)
        except (ValueError, TypeError) as e:
            raise e
        except errors.PyMongoError:
            raise HTTPException(status_code=500, detail="Error inserting notification")

    async def get_by_id(self, notification_id: str) -> Optional[NotificationResponseSchema]:
    ...
    async def delete(self, notification_id: str) -> None:
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f'Erro ao excluir notificação com ID {notification_id}: {e}'
            )
